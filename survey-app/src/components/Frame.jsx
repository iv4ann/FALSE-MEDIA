      ...
      <div className="w-full max-w-5xl bg-black border-2 border-[#ff3f14] shadow-[8px_8px_0px_#ff3f14,0_0_40px_rgba(255,63,20,0.4)] flex flex-col md:flex-row min-h-[620px] relative overflow-hidden my-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {!isLogin && (
                <div>
                  ...
                  <input
                    type="text"
                    required
                    className="w-full px-4 py-2 bg-white/5 border border-white/20 focus:border-[#ff3f14] focus:outline-none text-white font-vt323 text-xl"
                ...
                <label className="flex items-start gap-3 cursor-pointer select-none group">
                  <div className="relative flex items-center justify-center mt-1">
                    ...
                    <div className={`w-5 h-5 border-2 transition-all flex items-center justify-center ${
                      consent ? 'bg-[#ff3f14] border-[#ff3f14]' : 'bg-transparent border-white/50'
                    }`}>
                  ...
                  <span className="text-lg font-vt323 text-white/80 leading-tight">
                    Doy mi consentimiento informado para participar voluntariamente en esta prueba.
                  </span>
                </label>
              </div>
              ...
              <div className="pt-4">
                <button 
                  ...
                  className={`py-2 px-8 font-silkscreen text-xl transition-all ${
                    consent && !loading ? 'bg-[#ff3f14] text-white hover:bg-white hover:text-black cursor-pointer' : 'bg-white/10 text-white/30 cursor-not-allowed'
                  }`}
                >
        ...
        <div className="hidden md:block md:w-2/5 relative border-l-2 border-[#ff3f14] bg-black">
          ...
          <div className="absolute bottom-8 left-0 right-0 text-center z-30">
             <div className="inline-block px-4 py-1 border border-[#ff3f14] bg-black/80 font-vt323 text-[#ff3f14] text-2xl">
               SISTEMA SEGURO NEON DB
             </div>
          </div>
        </div>
      </div>
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 font-sans">
      <div className="w-full max-w-md bg-black border-2 border-[#ff3f14] p-6 shadow-[8px_8px_0px_#ff3f14]">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={onClose} className="font-vt323 text-white text-xl">[X]</button>
        </div>
...
export const Frame = ({ respuestasMultimedia, setRespuestasMultimedia, onSubmitReal }) => {
  const [activeModal, setActiveModal] = useState(null); // 'login' | 'signup' | 'edit'
        ...
        <header className="w-full h-[80px] shrink-0 bg-[#000000cc] border-b border-white/10 flex items-center justify-between px-10 shadow-[0_4px_30px_rgba(0,0,0,0.8)] z-50">
          <div className="flex items-center gap-8">
            <button 
              ...
              className="text-[#ff3f14] font-silkscreen text-2xl hover:text-white hover:scale-105 transition-all cursor-pointer"
            >
              ← INICIO
            </button>
      ...
      <div className="absolute top-0 left-0 w-full h-[80px] bg-[#000000cc] border-b border-white/10 flex items-center justify-end px-10 z-50 shadow-[0_4px_30px_rgba(0,0,0,0.8)] backdrop-blur-md">
        <div className="flex items-center gap-4">
          {user ? (
            ...
            <>
              <button onClick={() => setActiveModal('login')} className="font-vt323 text-white text-[26px] hover:text-[#ff3f14] cursor-pointer">Log in</button>
              <button onClick={() => setActiveModal('signup')} className="bg-white text-black px-5 py-1.5 rounded font-vt323 text-[26px] hover:bg-[#ff3f14] hover:text-white cursor-pointer">Sign up</button>
            </>
          )}
        </div>
      </div>

      <main className="w-full h-full overflow-x-hidden relative overflow-y-auto">
        <div className="relative w-full h-[2048px] flex justify-center">
          <img className="absolute top-0 left-0 w-full h-[1159px] object-cover pointer-events-none z-0" alt="Retro" src={unsplashEzen4Jyrvyq} />
          <div className="absolute top-[1157px] left-0 w-full h-[1024px] bg-black shadow-[0px_4px_122px_155px_#00000040] pointer-events-none z-0" />
              ...
              <nav className="absolute top-[660px] left-[390px] w-[724px] h-[57px] flex items-center justify-center gap-[60px] bg-[#ff3f14] rounded-md shadow-[4px_4px_0px_#000000bf] pointer-events-auto">
                {navigationItems.map((item) => (
                  <button
                    key={item.label}
                    type="button"
                    className="flex items-center gap-4 font-vt323 text-white text-2xl hover:scale-105 transition-transform cursor-pointer"
                    onClick={() => setActiveNavigation(item.label)}
                  >
                    {item.label}
                    <img className="w-[8px] h-[10px]" alt="" src={item.icon} />
                  </button>
                ))}
              </nav>
              <h1 className="absolute top-[134px] left-[122px] m-0 font-silkscreen text-white text-[61px] leading-[73.2px]">FALSE MEDIA</h1>
            </header>
