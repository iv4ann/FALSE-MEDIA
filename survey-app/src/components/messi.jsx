    ...
    <div className="w-full h-screen bg-black flex justify-center items-center overflow-hidden font-sans">
      <main className="relative w-[1440px] h-[1024px] overflow-hidden shrink-0 bg-black shadow-2xl">
        <div className="absolute inset-0 z-0">
          <img className="w-full h-full object-cover opacity-60" alt="Fondo Retro Televisores" src={unsplashEzen4Jyrvyq} />
        ...
        <section className="absolute top-[150px] left-[100px] w-[540px] z-20 flex flex-col gap-4">
          ...
          <div className={`transition-all duration-300 overflow-hidden ${intentoCambio ? 'max-h-10 opacity-100' : 'max-h-0 opacity-0'}`}>
            <div className="p-1.5 bg-red-900/60 border border-red-500/50 text-red-200 font-vt323 text-lg rounded-lg text-center">No se puede modificar la respuesta una vez enviada.</div>
          </div>
        </section>
          ...
          {getVisibleItems().map((item, index) => {
            ...
            return (
              <div
                ...
                className={`relative shrink-0 overflow-hidden cursor-pointer transition-all duration-500 ease-out shadow-[10px_10px_30px_rgba(0,0,0,0.9)] border border-white/10 ${isFirst ? "w-[320px] h-[440px] rounded-[10px] scale-100 opacity-100 border-white/40 ring-2 ring-[#ff3f14]/50" : "w-[220px] h-[340px] rounded-[10px] opacity-40 hover:opacity-80 hover:-translate-y-2"}`}
              >
