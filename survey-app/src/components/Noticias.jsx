...
export default function Noticias({ respuestas, setRespuestas }) {
  ...
  const getVisibleItems = () => [
    ...noticiasData.slice(activeIndex),
      ...
      <main className="relative w-[1440px] h-[1024px] overflow-hidden shrink-0 bg-black shadow-2xl">
        ...
        <section className="absolute top-[300px] left-[760px] flex items-center gap-[30px] z-20">
          {getVisibleItems().map((item, index) => (
            <div key={item.id} onClick={() => { setActiveIndex(noticiasData.findIndex((i) => i.id === item.id)); setIntentoCambio(false); }} className={`relative shrink-0 overflow-hidden cursor-pointer transition-all duration-500 ease-out shadow-[10px_10px_30px_rgba(0,0,0,0.9)] border border-white/10 ${index === 0 ? "w-[320px] h-[440px] rounded-[10px] scale-100 opacity-100 border-white/40 ring-2 ring-[#ff3f14]/50" : "w-[220px] h-[340px] rounded-[10px] opacity-40 hover:opacity-80 hover:-translate-y-2"}`}>
